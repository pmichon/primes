...
def oblicz_gestosc_w_przedziałach(pierwsze: Set[int], max_zakres: int, rozmiar_przedzialu: int = 10000) -> Tuple[List[int], List[float], List[int]]:
    """
    ...
    liczba_przedziałów = (max_zakres - 2) // rozmiar_przedzialu + (1 if (max_zakres - 2) % rozmiar_przedzialu else 0)
    ...
    for start in range(2, max_zakres, rozmiar_przedzialu):
        ...
        # Wyświetl postęp
        if aktualny_przedział % max(1, liczba_przedziałów // 20) == 0 or aktualny_przedział == liczba_przedziałów:
...
def utworz_wykres_gestosci(
    ...
    ax1.plot(przedzialy, gestosci_teoretyczne, 'r--', linewidth=2, alpha=0.7, label='Teoretyczna gęstość (1/ln(x))')
    
    ax1.set_xlabel('Liczba (środek przedziału)', fontsize=12)
    ax1.set_ylabel('Gęstość liczb pierwszych (%)', fontsize=12)
    ax1.set_title(f'Gęstość liczb pierwszych w przedziałach po {rozmiar_przedzialu:,}', fontsize=14, fontweight='bold')
...
def wyswietl_statystyki_gestosci(przedzialy: List[int], gestosci: List[float], 
                                gestosci_teoretyczne: List[float], liczby_w_przedziałach: List[int],
                                rozmiar_przedzialu: int, max_sprawdzone: int):
...
Przykłady użycia:
  ...
  %(prog)s --pokaz                  # Pokaż wykres na ekranie
    ...
    parser.add_argument('--plik-cache', default=PLIK_CACHE_PIERWSZYCH,
    ...
    try:
        ...
        if max_zakres < args.przedział * 2:
            print(f"⚠️  Ostrzeżenie: Zakres analizy ({max_zakres:,}) jest bardzo mały w porównaniu do rozmiaru przedziału ({args.przedział:,})")
