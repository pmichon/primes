...
def wczytaj_cache_do_sprawdzenia(nazwa_pliku: str = PLIK_CACHE_PIERWSZYCH) -> Tuple[Set[int], int, Dict]:
...
def sprawdz_poprawnosc_pierwszosci(pierwsze: Set[int], max_limit: int = None) -> Dict[str, List[int]]:
    ...
    if max_limit:
        ...
        print(f"Sprawdzanie {len(pierwsze_do_sprawdzenia):,} liczb pierwszych (limit: {max_limit:,})")
    else:
...
def sprawdz_kompletnosc(pierwsze: Set[int], max_sprawdzone: int, limit_sprawdzania: int = None) -> Dict[str, List[int]]:
    """Sprawdź czy nie brakuje liczb pierwszych w zakresie."""
    print(f"\n=== SPRAWDZANIE KOMPLETNOŚCI ===")
    
    effective_limit = min(max_sprawdzone, limit_sprawdzania) if limit_sprawdzania else max_sprawdzone
    print(f"Sprawdzanie kompletności do {effective_limit:,}")
    
    if effective_limit > 10**7:
        print(f"Ostrzeżenie: limit {effective_limit:,} jest bardzo duży, sprawdzanie może zająć długo.")
...
def sprawdz_strukture_cache(dane: Dict) -> Dict[str, any]:
    ...
    if isinstance(pierwsze, (set, list)) and pierwsze:
        ...
        if max_w_cache > max_sprawdzone:
            problemy.append(f"Największa liczba w cache ({max_w_cache:,}) > max_sprawdzone ({max_sprawdzone:,})")
        elif max_w_cache < max_sprawdzone * 0.9:  # Tolerancja 10%
            ostrzezenia.append(f"Największa liczba w cache ({max_w_cache:,}) znacznie mniejsza od max_sprawdzone ({max_sprawdzone:,})")
    ...
    if isinstance(pierwsze, (set, list)):
        ...
        if nieprawidlowe_wartosci:
            problemy.append(f"Znaleziono {len(nieprawidlowe_wartosci)} nieprawidłowych wartości: {nieprawidlowe_wartosci[:10]}")
    ...
    if pierwsze:
        ...
        if max_sprawdzone > 1:
            ...
            if max_sprawdzone > 10:
                ...
                print(f"Różnica od szacowanej: {roznica:+,.0f} ({roznica/szacowana_liczba*100:+.1f}%)")
